%bf_init.m ; part of the Brightfield GUI: execute bf_main.m
%version 2.0.1
%written by O. Schwab (oschwab@stanford.edu)
%last change: 01/04/15

function bf_init(h_main)
%main function for the initilization window of bf gui
...
h_init(1).fig=figure(...
    ...
    'Name','Brightfield Initialization',...
    'NumberTitle','off',...
    'Resize','off',...
    'Color',[.94,.94,.94],...
    'visible','off');
...
%and show parameters
if ~isempty(getappdata(0,'bf_init_user_filenamestack'))                          %files on stack
    %load shared parameters
    bf_init_user_counter=getappdata(0,'bf_init_user_counter');                %current video number
    bf_init_user_Nfiles=getappdata(0,'bf_init_user_Nfiles');                  %number of videos
    bf_init_user_conversion=getappdata(0,'bf_init_user_conversion');          %conversion factor: 1px=?micron
    bf_init_user_framerate=getappdata(0,'bf_init_user_framerate');            %framerate
    bf_init_user_Nframes=getappdata(0,'bf_init_user_Nframes');                %number of frames
    bf_init_user_cellname=getappdata(0,'bf_init_user_cellname');              %cellname
    bf_init_user_filenamestack=getappdata(0,'bf_init_user_filenamestack');    %filestack
    bf_init_user_vidext=getappdata(0,'bf_init_user_vidext');                  %video extension
    bf_init_user_preview_frame1=getappdata(0,'bf_init_user_preview_frame1');  %preview frame for each video
    
    %change display strings and values
    %put video files into listbox
    set(h_init(1).listbox_display,'String',bf_init_user_filenamestack);
    %select item in listbox
    set(h_init(1).listbox_display,'Value',bf_init_user_counter);
    %display 1st frame of new video
    axes(h_init(1).axes_curr);
    imshow(bf_init_user_preview_frame1{bf_init_user_counter});
    %display new settings
    set(h_init(1).edit_fps,'String',num2str(bf_init_user_framerate{bf_init_user_counter}));                               %framerate
    set(h_init(1).edit_conversion,'String',num2str(bf_init_user_conversion{bf_init_user_counter}));                       %conversion
    set(h_init(1).edit_nframes,'String',num2str(bf_init_user_Nframes{bf_init_user_counter}));                             %number of frames
    set(h_init(1).edit_cellname,'String',bf_init_user_cellname{bf_init_user_counter});                                    %cellname
    set(h_init(1).text_whichvidname,'String',[bf_init_user_filenamestack{1,bf_init_user_counter},' (showing 1st frame)']);%name of video
    set(h_init(1).text_whichvid,'String',[num2str(bf_init_user_counter),'/',num2str(bf_init_user_Nfiles)]);               %which video (i/N)
    
    %set/change properties
    %if tif or avi or folder, user needs to edit edits; disable nframes
    if strcmp(bf_init_user_vidext{1,bf_init_user_counter},'.tif') || strcmp(bf_init_user_vidext{1,bf_init_user_counter},'.avi') || strcmp(bf_init_user_vidext{1,bf_init_user_counter},'none')
    ...
    if bf_init_user_counter==bf_init_user_Nfiles; %if current vid is the last on stack
        set(h_init(1).button_forwards,'Enable','off');  %disable forwards
    else
        set(h_init(1).button_forwards,'Enable','on');   %enable forwards
    end
    if bf_init_user_counter==1;                      %if current vid is the first on stack
...
try
    ...
    bf_init_user_vidext=getappdata(0,'bf_init_user_vidext');
    ...
    %bf_init_user_mask=getappdata(0,'bf_init_user_mask');
    ...
    Nfiles0=size(bf_init_user_filenamestack,2);
    
    %add new files and paths to stacks
    for i=1:size(filename,2)
        [~,name,ext]=fileparts(strcat(pathname,filename{1,i}));
        bf_init_user_filenamestack{1,Nfiles0+i}=name;
        bf_init_user_pathnamestack{1,Nfiles0+i}=pathname;
        bf_init_user_vidext{1,Nfiles0+i}=ext;
    end
    ...
    %new number of files
    bf_init_user_Nfiles=size(bf_init_user_filenamestack,2);
    ...
    for j=1:size(filename,2)
        ...
        if isequal(exist(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+j}], 'dir'),7)
            rmdir(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+j}],'s')
        end
        mkdir(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+j}])
        
        %check format, load and save:
        if strcmp(bf_init_user_vidext{1,Nfiles0+j},'.czi')
            %use bioformats for import
            [~,data]=evalc('bfopen([bf_init_user_pathnamestack{1,Nfiles0+j},bf_init_user_filenamestack{1,Nfiles0+j},bf_init_user_vidext{1,Nfiles0+j}]);');
            ...
            for i=1:N
                ...
                %save to mat
                save(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+j},'/image',num2str(i),'.mat'],'imagei','-v7.3')
            end
            
        elseif strcmp(bf_init_user_vidext{1,Nfiles0+j},'.tif')
            InfoImage=imfinfo([bf_init_user_pathnamestack{1,Nfiles0+j},bf_init_user_filenamestack{1,Nfiles0+j},bf_init_user_vidext{1,Nfiles0+j}]);
            N=length(InfoImage);
            TifLink = Tiff([bf_init_user_pathnamestack{1,Nfiles0+j},bf_init_user_filenamestack{1,Nfiles0+j},bf_init_user_vidext{1,Nfiles0+j}], 'r');
            for i=1:N
        ...
        elseif strcmp(bf_init_user_vidext{1,Nfiles0+j},'.avi')
            videoObj = VideoReader([bf_init_user_pathnamestack{1,Nfiles0+j},bf_init_user_filenamestack{1,Nfiles0+j},bf_init_user_vidext{1,Nfiles0+j}]);
        ...
        s=load(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+j},'/image1.mat'],'imagei');
        bf_init_user_preview_frame1{Nfiles0+j}=normalise(s.imagei);
        
        %save mask: initially all ones:
        %bf_init_user_mask{Nfiles0+j}=ones(size(s.imagei,1),size(s.imagei,2));
        ...
        if strcmp(bf_init_user_vidext{1,Nfiles0+j},'.czi')
            ...
            %for later use:
            bf_init_user_conversion{Nfiles0+j}=voxelSizeX;
            bf_init_user_framerate{Nfiles0+j}=1/tincrement;
            bf_init_user_Nframes{Nfiles0+j}=N;
            bf_init_user_cellname{Nfiles0+j}='cellname1';
            
            %if tiff or avi
        elseif strcmp(bf_init_user_vidext{1,Nfiles0+j},'.tif') || strcmp(bf_init_user_vidext{1,Nfiles0+j},'.avi')
            bf_init_user_conversion{Nfiles0+j}=NaN;
            bf_init_user_framerate{Nfiles0+j}=NaN;
            bf_init_user_Nframes{Nfiles0+j}=N;
            bf_init_user_cellname{Nfiles0+j}='cellname1';
        end
    end
    ...
    set(h_init(1).edit_fps,'String',num2str(bf_init_user_framerate{1}));
    set(h_init(1).edit_conversion,'String',num2str(bf_init_user_conversion{1}));
    if strcmp(bf_init_user_vidext{1,1},'.tif') || strcmp(bf_init_user_vidext{1,1},'.avi') || strcmp(bf_init_user_vidext{1,1},'none')
    ...
    set(h_init(1).edit_nframes,'String',num2str(bf_init_user_Nframes{1}));
    set(h_init(1).edit_nframes,'Enable','off');
    
    set(h_init(1).edit_cellname,'String',bf_init_user_cellname{1});
    set(h_init(1).text_whichvidname,'String',[bf_init_user_filenamestack{1,1},' (showing 1st frame)']);
    
    %set frame1 of 1st video
    axes(h_init(1).axes_curr);
    imshow(bf_init_user_preview_frame1{1});
    
    %select item in listbox
    set(h_init(1).listbox_display,'Value',1);
    
    %update text (x/N)
    set(h_init(1).text_whichvid,'String',[num2str(1),'/',num2str(bf_init_user_Nfiles)]);
    
    %forwards and backwards buttons enable/disable
    if 1==bf_init_user_Nfiles; %if current vid is the last on stack
        set(h_init(1).button_forwards,'Enable','off');  %disable forwards
    else
        set(h_init(1).button_forwards,'Enable','on');   %enable forwards
    ...
    bf_init_user_counter=1;
    
    %store everything for shared use
    setappdata(0,'bf_init_user_filenamestack',bf_init_user_filenamestack)
    setappdata(0,'bf_init_user_pathnamestack',bf_init_user_pathnamestack)
    setappdata(0,'bf_init_user_vidext',bf_init_user_vidext)
    setappdata(0,'bf_init_user_counter',bf_init_user_counter)
    setappdata(0,'bf_init_user_Nfiles',bf_init_user_Nfiles)
    setappdata(0,'bf_init_user_cellname',bf_init_user_cellname)
    setappdata(0,'bf_init_user_Nframes',bf_init_user_Nframes)
    setappdata(0,'bf_init_user_conversion',bf_init_user_conversion)
    setappdata(0,'bf_init_user_framerate',bf_init_user_framerate)
    %setappdata(0,'bf_init_user_mask',bf_init_user_mask)
    setappdata(0,'bf_init_user_preview_frame1',bf_init_user_preview_frame1)
    ...
    %display anything
    if isempty(getappdata(0,'bf_init_user_filenamestack'))
...
try
    ...
    %add new files and paths to stacks
    bf_init_user_filenamestack{1,Nfiles0+1}=name{1};
    bf_init_user_pathnamestack{1,Nfiles0+1}=foldername;
    bf_init_user_vidext{1,Nfiles0+1}='none';
    ...
    %create folder for saving the stuff
    if isequal(exist(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+1}], 'dir'),7)
        rmdir(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+1}],'s')
    end
    mkdir(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+1}])
    ...
    for i = 1:N
        ...
        imagei=normalise(imagei);
        %save to mat
        save(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+1},'/image',num2str(i),'.mat'],'imagei','-v7.3')
    end
    
    
    
    %save preview of 1st frame in workspace for display
    s=load(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{1,Nfiles0+1},'/image1.mat'],'imagei');
    bf_init_user_preview_frame1{Nfiles0+1}=normalise(s.imagei);
    
    %save mask: initially all ones:
    %bf_init_user_mask{Nfiles0+1}=ones(size(s.imagei,1),size(s.imagei,2));
    
    %metadata to NaN
    bf_init_user_conversion{Nfiles0+1}=NaN;
    bf_init_user_framerate{Nfiles0+1}=NaN;
    bf_init_user_Nframes{Nfiles0+1}=N;
    bf_init_user_cellname{Nfiles0+1}='cellname1';
...
try
    ...
    if index==bf_init_user_Nfiles
        index_next=index-1;
    else index_next=index;
    end
    
    %remove directory of saved imagedata
    if isequal(exist(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{index}], 'dir'),7)
        rmdir(['vars_DO_NOT_DELETE/',bf_init_user_filenamestack{index}],'s')
    end
    
    %delete from saved stuff
    bf_init_user_Nfiles=bf_init_user_Nfiles-1;
    bf_init_user_conversion(index)=[];
    bf_init_user_cellname(index)=[];
    bf_init_user_framerate(index)=[];
    bf_init_user_Nframes(index)=[];
    bf_init_user_filenamestack(index)=[];
    bf_init_user_pathnamestack(index)=[];
    bf_init_user_vidext(index)=[];
    bf_init_user_preview_frame1(index)=[];
    %bf_init_user_mask(index)=[];
    set(h_init(1).listbox_display,'String',bf_init_user_filenamestack);
    
    if ~isempty(bf_init_user_filenamestack)
        %look at vid counter
        bf_init_user_counter=index_next;
        ...
        %display new settings
        set(h_init(1).edit_fps,'String',num2str(bf_init_user_framerate{bf_init_user_counter}));
        set(h_init(1).edit_conversion,'String',num2str(bf_init_user_conversion{bf_init_user_counter}));
        set(h_init(1).edit_nframes,'String',num2str(bf_init_user_Nframes{bf_init_user_counter}));
        set(h_init(1).edit_nframes,'Enable','off');
        set(h_init(1).edit_cellname,'String',bf_init_user_cellname{bf_init_user_counter});
        set(h_init(1).text_whichvidname,'String',[bf_init_user_filenamestack{1,bf_init_user_counter},' (showing 1st frame)']);
        if strcmp(bf_init_user_vidext{1,bf_init_user_counter},'.tif') || strcmp(bf_init_user_vidext{1,bf_init_user_counter},'.avi') || strcmp(bf_init_user_vidext{1,bf_init_user_counter},'none')
            set(h_init(1).edit_fps,'Enable','on');
            set(h_init(1).edit_conversion,'Enable','on');
        else
        ...
        %update text (x/N)
        set(h_init(1).text_whichvid,'String',[num2str(bf_init_user_counter),'/',num2str(bf_init_user_Nfiles)]);
        
        if bf_init_user_counter==1;
            set(h_init(1).button_backwards,'Enable','off');
        else
            set(h_init(1).button_backwards,'Enable','on');
        end
        if bf_init_user_counter==bf_init_user_Nfiles;
        set(h_init(1).button_forwards,'Enable','off');
        else
        set(h_init(1).button_forwards,'Enable','on');
        end
...
try
    ...
    %load what shared para we need
    bf_init_user_counter=getappdata(0,'bf_init_user_counter');
    bf_init_user_Nfiles=getappdata(0,'bf_init_user_Nfiles');
    bf_init_user_conversion=getappdata(0,'bf_init_user_conversion');
    bf_init_user_framerate=getappdata(0,'bf_init_user_framerate');
    bf_init_user_Nframes=getappdata(0,'bf_init_user_Nframes');
    ...
    if bf_init_user_counter<bf_init_user_Nfiles;
        %save settings
        bf_init_user_framerate{bf_init_user_counter}=str2double(get(h_init(1).edit_fps,'String'));
        bf_init_user_conversion{bf_init_user_counter}=str2double(get(h_init(1).edit_conversion,'String'));
        bf_init_user_cellname{bf_init_user_counter}=get(h_init(1).edit_cellname,'String');
        
        %go to video before
        bf_init_user_counter=bf_init_user_counter+1;
...
% 
% %save as new mask
% bf_init_user_mask{bf_init_user_counter}=binaryImage;
% 
% %display preview w. mask
% cla(h_init(1).axes_curr)
% axes(h_init(1).axes_curr)
% imshow(bf_init_user_preview_frame1{bf_init_user_counter}.*bf_init_user_mask{bf_init_user_counter});
% 
% %save
% setappdata(0,'bf_init_user_mask',bf_init_user_mask)
% catch errorObj
...
% 
% %save as new mask
% bf_init_user_mask{bf_init_user_counter}=var.mask;
% 
% %display preview w. mask
% cla(h_init(1).axes_curr)
% axes(h_init(1).axes_curr)
...
for ivid=1:bf_init_user_Nfiles
    if ~isempty(find(isnan([bf_init_user_framerate{:}]))) || ~isempty(find(isnan([bf_init_user_conversion{:}])))
...
for ivid=1:bf_init_user_Nfiles
    sb=statusbar(h_init(1).fig,['Saving... ',num2str(floor(100*(ivid-1)/bf_init_user_Nfiles)), '%% done']);
    sb.getComponent(0).setForeground(java.awt.Color.red);
    %create folder ewith same name as vid. & mask folder
    if ~isequal(exist([bf_init_user_pathnamestack{1,ivid},bf_init_user_filenamestack{1,ivid},'/Results'], 'dir'),7)
        mkdir([bf_init_user_pathnamestack{1,ivid},bf_init_user_filenamestack{1,ivid},'/Results'])
    end
    ...
    A = {bf_init_user_framerate{ivid},bf_init_user_conversion{ivid},bf_init_user_Nframes{ivid},bf_init_user_cellname{ivid}};
    sheet = 'General';
    xlRange = 'A3';
    xlwrite(newfile,A,sheet,xlRange)
    
    %save mask
    %mask=bf_init_user_mask{ivid};
