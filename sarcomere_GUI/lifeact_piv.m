%lifeact_piv.m ; part of the Lifeact GUI: execute lifeact_main.m
%version 2.0.1
%written by O. Schwab (oschwab@stanford.edu)
%last change: 01/04/15

function lifeact_piv(h_main)
%main function for the displacement window of lifeact gui
...
try    
    ...
    imshow(sarco_init_user_preview_frame1{1});hold on;
    
    %initiate counter (which video)
    sarco_piv_user_counter=1;
    
    %initialize relax/contr/dis
    sarco_piv_user_contr=cell(1,sarco_init_user_Nfiles);
    sarco_piv_user_relax=cell(1,sarco_init_user_Nfiles);
    sarco_piv_user_d=cell(1,sarco_init_user_Nfiles);
    
    %store everything for shared use
    setappdata(0,'sarco_piv_user_counter',sarco_piv_user_counter)
    setappdata(0,'sarco_piv_user_contr',sarco_piv_user_contr);
    setappdata(0,'sarco_piv_user_relax',sarco_piv_user_relax);
    setappdata(0,'sarco_piv_user_d',sarco_piv_user_d);
...
%     %ncorr settings
    ...
    for ivid=1:sarco_init_user_Nfiles
        ...
        if ~isequal(exist([sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Plots/Displacement Heatmaps'], 'dir'),7)
                mkdir([sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Plots/Displacement Heatmaps'])
        end
        ...
        for ifr=1:sarco_init_user_Nframes{ivid}
            ...
            witer=witer+1;
            sb=statusbar(h_piv(1).fig,['Tweaking images... ',num2str(floor(100*(witer-1)/sum([sarco_init_user_Nframes{:}]))), '%% done']);
            sb.getComponent(0).setForeground(java.awt.Color.red);
            
            s=load(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/image',num2str(ifr),'.mat'],'imagei');
            im=s.imagei;
            imwrite(im,['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/image',num2str(ifr),'.png']);
        end
    end
    %statusbar
    ...
    %loop over videos
    for ivid=1:sarco_init_user_Nfiles
        sb=statusbar(h_piv(1).fig,['Calculating... ',num2str(floor(100*(ivid-1)/sarco_init_user_Nfiles)), '%% done']);
        sb.getComponent(0).setForeground(java.awt.Color.red);
        
        %set reference
        ref = ncorr_class_img;
        ref.set_img('lazy',struct('name','image1.png','path',['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid}]));
        
        %set current
        cur = ncorr_class_img.empty;
        for ifr=1:sarco_init_user_Nframes{ivid}
            cur(end+1) = ncorr_class_img;
            cur(end).set_img('lazy',struct('name',['image',num2str(ifr),'.png'],'path',['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid}]));
        end
        
        %set roi: take convex hull to avoid problems with tiny particles at
        %edges 
        mask = bwconvhull(sarco_conc_user_binary1{ivid});
        roi = ncorr_class_roi;
        ...
        for frame = 1:length(cur)
            ...
            s=load(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/image',num2str(1),'.mat'],'imagei');
            im_ref=s.imagei;
            ...
            %clear downstream of ncorr
            %clear_downstream();
            
            %save displaced cell roi
            %dicinfo(frame)
            %(frame).roi_dic;
            %save(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/deformed_mask_',num2str(frame),'.mat'],'defmask','-v7.3')
            ...
            %save to mat
            save(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_x_',num2str(frame),'.mat'],'x','-v7.3')
            save(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_y_',num2str(frame),'.mat'],'y','-v7.3')
            save(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_u_',num2str(frame),'.mat'],'fu','-v7.3')
            save(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_v_',num2str(frame),'.mat'],'fv','-v7.3')
            %sb=statusbar(h_piv(1).fig,['Calculating... ',num2str(floor(100*(witer-1)/sum([sarco_init_user_Nframes{:}]))), '%% done']);
            %sb.getComponent(0).setForeground(java.awt.Color.red);
            %current image
            ...
            set(gca,'linewidth',2);
            axis image;
            export_fig([sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Plots/Displacement Heatmaps/heatmap',int2str(frame)],'-png','-m2',p);
            close(p)
            %save data in text files
            %save([sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/displacements/dis', int2str(ifr),'.mat'],'x','y','u_filt','v_filt');
        end
...
try
    ...
    sarco_piv_user_d=getappdata(0,'sarco_piv_user_d');
    sarco_piv_user_counter=getappdata(0,'sarco_piv_user_counter');
    sarco_conc_user_binary1=getappdata(0,'sarco_conc_user_binary1');
    
    
    for current_vid=1:sarco_init_user_Nfiles
        %waitbar
        sb=statusbar(h_piv(1).fig,['Calculating... ',num2str(floor(100*(current_vid-1)/sarco_init_user_Nfiles)), '%% done']);
        sb.getComponent(0).setForeground(java.awt.Color.red);
        
        %get displacmenets from calc. before
        Num=sarco_init_user_Nframes{current_vid};
        sarco_piv_user_xs=cell(1,Num);
        sarco_piv_user_ys=cell(1,Num);
        sarco_piv_user_us=cell(1,Num);
        sarco_piv_user_vs=cell(1,Num);
        
        for ifr=1:Num
            s=load(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,current_vid},'/piv_x_',num2str(ifr),'.mat'],'x');
            sarco_piv_user_xs{ifr}=s.x;
            s=load(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,current_vid},'/piv_y_',num2str(ifr),'.mat'],'y');
            sarco_piv_user_ys{ifr}=s.y;
            s=load(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,current_vid},'/piv_u_',num2str(ifr),'.mat'],'fu');
            sarco_piv_user_us{ifr}=s.fu;
            s=load(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,current_vid},'/piv_v_',num2str(ifr),'.mat'],'fv');
            sarco_piv_user_vs{ifr}=s.fv;
        end
        
        %mask: translate to correct spacing 
        dismask=sarco_conc_user_binary1{current_vid}(sarco_piv_user_ys{1}(:,1) ,sarco_piv_user_xs{1}(1,:));
              
        %smartguess reference
        [relax,contr] = smartguess_reference(Num,sarco_piv_user_us,sarco_piv_user_vs,dismask);
        %for later use
        sarco_piv_user_contr{current_vid}=contr;
        sarco_piv_user_relax{current_vid}=relax;
        ...
        u1ref=sarco_piv_user_us{relax};
        v1ref=sarco_piv_user_vs{relax};
        d=zeros(1,Num);
        for ktest=1:Num
            deltaU=mask.*(sarco_piv_user_us{ktest}-u1ref);
            deltaV=mask.*(sarco_piv_user_vs{ktest}-v1ref);
            usn=(sarco_piv_user_us{ktest}-u1ref)-nanmean(deltaU(:)).*ones(size(u1ref,1),size(u1ref,2));
            vsn=(sarco_piv_user_vs{ktest}-v1ref)-nanmean(deltaV(:)).*ones(size(v1ref,1),size(v1ref,2));
            dsn=mask.*sqrt(usn.^2+vsn.^2);
            d(ktest)=nanmean(dsn(:));
        end
        d(relax)=NaN;
        %for later use
        sarco_piv_user_d{current_vid}=d*sarco_init_user_conversion{current_vid}*1e-6;
    end
    %statusbar
    sb=statusbar(h_piv(1).fig,'Calculation - Done !');
    sb.getComponent(0).setForeground(java.awt.Color(0,.5,0));
    ...
    set(h_piv(1).edit_ref_ref,'String',num2str(sarco_piv_user_relax{sarco_piv_user_counter}));
    set(h_piv(1).edit_ref_contr,'String',num2str(sarco_piv_user_contr{sarco_piv_user_counter}));
    
    %display displacement plot
    reset(h_piv(1).axes_ref)
    axes(h_piv(1).axes_ref);
    plot(1:length(sarco_piv_user_d{sarco_piv_user_counter}),sarco_piv_user_d{sarco_piv_user_counter}*1e6)
    set(h_piv(1).axes_ref, 'XTick', []);
    set(h_piv(1).axes_ref, 'YTick', []);
    
    %buttons
    %set(h_piv(1).button_ok,'Visible','on');
    if sarco_piv_user_counter>1
        set(h_piv(1).button_backwards,'Enable','on');
    else
        set(h_piv(1).button_backwards,'Enable','off');
    end
    if sarco_piv_user_counter==sarco_init_user_Nfiles
    ...
    set(h_piv(1).text_whichvidname,'String',sarco_init_user_filenamestack{1,sarco_piv_user_counter});
    set(h_piv(1).text_whichvid,'String',[num2str(sarco_piv_user_counter),'/',num2str(sarco_init_user_Nfiles)]);
...
try
    ...
    %open figure
    hf=figure;
    plot((1:length(sarco_piv_user_d{sarco_piv_user_counter})),sarco_piv_user_d{sarco_piv_user_counter})
    hold on;
    title('Pick relaxed')
    ...
    sarco_piv_user_relax{sarco_piv_user_counter}=x;
...
try
    ...
    %save as new reference
    sarco_piv_user_contr{sarco_piv_user_counter}=x;
...
try
    ...
    current_vid=sarco_piv_user_counter;
...
%angles
%initialize
sarco_piv_user_meand=cell(sarco_init_user_Nfiles,max([sarco_init_user_Nframes{:}]));
sarco_piv_user_meanalpha=cell(sarco_init_user_Nfiles,max([sarco_init_user_Nframes{:}]));
sarco_piv_user_stdalpha=cell(sarco_init_user_Nfiles,max([sarco_init_user_Nframes{:}]));
sarco_piv_user_meandelta=cell(sarco_init_user_Nfiles,max([sarco_init_user_Nframes{:}]));
sarco_piv_user_stddelta=cell(sarco_init_user_Nfiles,max([sarco_init_user_Nframes{:}]));
sarco_piv_user_delta=cell(1,sarco_init_user_Nfiles);
sarco_piv_user_meand_roi=cell(sarco_init_user_Nfiles,max(sarco_conc_user_number_roi),max([sarco_init_user_Nframes{:}]));
sarco_piv_user_meanalpha_roi=cell(sarco_init_user_Nfiles,max(sarco_conc_user_number_roi),max([sarco_init_user_Nframes{:}]));
sarco_piv_user_stdalpha_roi=cell(sarco_init_user_Nfiles,max(sarco_conc_user_number_roi),max([sarco_init_user_Nframes{:}]));
sarco_piv_user_meandelta_roi=cell(sarco_init_user_Nfiles,max(sarco_conc_user_number_roi),max([sarco_init_user_Nframes{:}]));
sarco_piv_user_stddelta_roi=cell(sarco_init_user_Nfiles,max(sarco_conc_user_number_roi),max([sarco_init_user_Nframes{:}]));
sarco_piv_user_delta_roi=cell(sarco_init_user_Nfiles,max(sarco_conc_user_number_roi));
...
for ivid=1:sarco_init_user_Nfiles
    ...
    %loop over frames
    Num=sarco_init_user_Nframes{ivid};
    %
    s=load(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_x_',num2str(1),'.mat'],'x');
    sarco_piv_user_xs=s.x;
    s=load(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_y_',num2str(1),'.mat'],'y');
    sarco_piv_user_ys=s.y;
    
    for frame=1:Num    
        %mask
        sarco_piv_user_dismask=sarco_conc_user_binary1{ivid}(sarco_piv_user_ys(:,1) ,sarco_piv_user_xs(1,:));
        
        %calculation
        [ displacement,alphamean,alphastd,displacement_roi,alphamean_roi,alphastd_roi,~]=calc_blobb3(sarco_init_user_filenamestack{1,ivid},sarco_piv_user_relax{ivid},sarco_piv_user_contr{ivid},frame,sarco_piv_user_dismask,sarco_conc_user_binary1{ivid},sarco_conc_user_counter_roi(ivid),sarco_conc_user_number_roi(ivid),sarco_conc_user_binary_roi(:,ivid));
        
        if frame==sarco_piv_user_relax{ivid}
            sarco_piv_user_meand{ivid,frame}=NaN;
        else
            sarco_piv_user_meand{ivid,frame}=displacement*sarco_init_user_conversion{ivid}*1e-6;
        end
        sarco_piv_user_meanalpha{ivid,frame}=alphamean;
        sarco_piv_user_stdalpha{ivid,frame}=alphastd;
        if sarco_conc_user_counter_roi(ivid)
            %loop over rois
            for roi=1:sarco_conc_user_number_roi(ivid)
                sarco_piv_user_meand_roi{ivid,roi,frame}=displacement_roi(roi)*sarco_init_user_conversion{ivid}*1e-6;
                sarco_piv_user_meanalpha_roi{ivid,roi,frame}=alphamean_roi(roi);
                sarco_piv_user_stdalpha_roi{ivid,roi,frame}=alphastd_roi(roi);
                %test
                %disp(num2str(sarco_piv_user_meanalpha_roi{ivid,roi,frame}))
            end
        end
    ...
    dis=cell(1,Num);
    for frame=1:Num
        s=load(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/dis_pts',num2str(frame),'.mat'],'dis_pts');
        dis{frame}=s.dis_pts;
    end
    
    %find peaks for mean displ. (inside cell)
    [pks,locs] = findpeaks([sarco_piv_user_meand{ivid,2:end-1}],'MINPEAKHEIGHT',.5*max([sarco_piv_user_meand{ivid,:}]));
    
    %find pts inside blobbs...
    blobb_comb=double(sarco_conc_user_binary1{ivid});
    if sarco_conc_user_counter_roi(ivid)
        %loop over rois
        for roi=1:sarco_conc_user_number_roi(ivid)
            blobb_comb=blobb_comb+double(sarco_conc_user_binary_roi{roi,ivid});
        end
    end
    ...
    for kk=1:length(xb)
        ...
        if max(di)>0 || min(di)<0
           ...
           s1=[sarco_piv_user_meand{ivid,:}];
           s2=di*sarco_init_user_conversion{ivid}*1e-6;
    ...
    binary0=double(sarco_conc_user_binary1{ivid});
    binary0(binary0==0)=NaN;
    
    sarco_piv_user_delta{ivid}=delta.*binary0;
    
    delta_vec = subsref((delta.*binary0).', substruct('()', {':'})).';
    delta_vec(isnan(delta_vec))=[];
    sarco_piv_user_meandelta{ivid}=mean(delta_vec);
    sarco_piv_user_stddelta{ivid}=std(delta_vec);
    ...
    if sarco_conc_user_counter_roi(ivid)
        %loop over rois
        for roi=1:sarco_conc_user_number_roi(ivid)
            binary0=double(sarco_conc_user_binary_roi{roi,ivid});
            binary0(binary0==0)=NaN;
            
            sarco_piv_user_delta_roi{ivid,roi}=delta.*binary0;
            
            delta_vec = subsref((delta.*binary0).', substruct('()', {':'})).';
            delta_vec(isnan(delta_vec))=[];
            sarco_piv_user_meandelta_roi{ivid,roi}=mean(delta_vec);
            sarco_piv_user_stddelta_roi{ivid,roi}=std(delta_vec);
            
            %test
            %disp(num2str(sarco_piv_user_stddelta_roi{ivid,roi}))
            %         figure,
            %         imshow(delta.*binary0,[])
            %         colormap('jet')
            %         colorbar;
...
%save for shared use
setappdata(0,'sarco_piv_user_meand',sarco_piv_user_meand)
setappdata(0,'sarco_piv_user_meanalpha',sarco_piv_user_meanalpha)
setappdata(0,'sarco_piv_user_stdalpha',sarco_piv_user_stdalpha)
setappdata(0,'sarco_piv_user_meandelta',sarco_piv_user_meandelta)
setappdata(0,'sarco_piv_user_stddelta',sarco_piv_user_stddelta)
setappdata(0,'sarco_piv_user_delta',sarco_piv_user_delta)
setappdata(0,'sarco_piv_user_meand_roi',sarco_piv_user_meand_roi)
setappdata(0,'sarco_piv_user_meanalpha_roi',sarco_piv_user_meanalpha_roi)
setappdata(0,'sarco_piv_user_stdalpha_roi',sarco_piv_user_stdalpha_roi)
setappdata(0,'sarco_piv_user_meandelta_roi',sarco_piv_user_meandelta_roi)
setappdata(0,'sarco_piv_user_stddelta_roi',sarco_piv_user_stddelta_roi)
setappdata(0,'sarco_piv_user_delta_roi',sarco_piv_user_delta_roi)
...
try
    ...
    %update counter 
    sarco_piv_user_counter=sarco_piv_user_counter-1;
...
try
    ...
    for ivid=1:sarco_init_user_Nfiles
        sb=statusbar(h_piv(1).fig,['Saving to Excel... ',num2str(floor(100*(ivid-1)/sarco_init_user_Nfiles)), '%% done']);
        sb.getComponent(0).setForeground(java.awt.Color.red);
        ...
        A = {sarco_piv_user_relax{ivid},sarco_piv_user_contr{ivid}};
        sheet = 'General';
        xlRange = 'F3';
    ...
    if value
        ...
        for ivid=1:sarco_init_user_Nfiles
            ...
            if ~isequal(exist([sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Datasets/Displacements'], 'dir'),7)
                mkdir([sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Datasets/Displacements'])
            end
            ...
            for ifr=1:sarco_init_user_Nframes{ivid}
                %waitbar
                w_i=w_i+1;
                sb=statusbar(h_piv(1).fig,['Saving full fields... ',num2str(floor(100*(w_i-1)/sum([sarco_init_user_Nframes{:}]))), '%% done']);
                sb.getComponent(0).setForeground(java.awt.Color.red);
                %copy displacements
                copyfile(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_x_',num2str(ifr),'.mat'],[sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Datasets/Displacements/piv_x_',num2str(ifr),'.mat']);
                copyfile(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_y_',num2str(ifr),'.mat'],[sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Datasets/Displacements/piv_y_',num2str(ifr),'.mat']);
                copyfile(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_u_',num2str(ifr),'.mat'],[sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Datasets/Displacements/piv_u_',num2str(ifr),'.mat']);
                copyfile(['vars_DO_NOT_DELETE/',sarco_init_user_filenamestack{1,ivid},'/piv_v_',num2str(ifr),'.mat'],[sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Datasets/Displacements/piv_v_',num2str(ifr),'.mat']);
            end
        end
        %statusbar
        sb=statusbar(h_piv(1).fig,'Saving - Done !');
    ...
    if ~value
        for ivid=1:sarco_init_user_Nfiles
        rmdir([sarco_init_user_pathnamestack{1,ivid},sarco_init_user_filenamestack{1,ivid},'/Plots/Displacement Heatmaps'],'s')
        end
    end
...
set(h_main(1).button_sarco,'Enable','on');
